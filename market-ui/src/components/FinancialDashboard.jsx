...
export default function FinancialDashboard() {
    ...
    const handleCompanyAdded = (symbol) => {
        console.log(`New company added/selected: ${symbol}`);
        setSelectedCompany(symbol);
        setTimeout(() => {
            refetch();
        }, 500);
...
function ControlPanel({
    ...
    limitType, setLimitType, customLimit, setCustomLimit,
    darkTheme, setDarkTheme, view, setView, onCompanyAdded,
    profitabilityView, setProfitabilityView
}) {
    // Get companies from database
    const { companies: dbCompanies, loading: loadingCompanies } = useAvailableCompanies();
    ...
    // Analysis view options (shown when view === 'analysis')
    ...
    return (
        <div style={darkTheme ? styles.unifiedToolbarDark : styles.unifiedToolbar}>
                ...
                {view === 'analysis' && (
                    <>
                        ...
                        {analysisViewOptions.map(option => (
                            <button
                                key={option.id}
                                onClick={() => setProfitabilityView(option.id)}
                                style={{
                                    ...(darkTheme ? styles.viewTabSmallDark : styles.viewTabSmall),
                                    ...(profitabilityView === option.id ? (darkTheme ? styles.viewTabSmallActiveDark : styles.viewTabSmallActive) : {})
                                }}
                                title={option.label}
                            >
            ...
            <div style={styles.toolbarRight}>
                ...
                {/* Company Select - Database driven */}
                <select
                    value={selectedCompany}
                    onChange={(e) => setSelectedCompany(e.target.value)}
                    style={darkTheme ? styles.toolbarSelectDark : styles.toolbarSelect}
...
function ComparisonView({ darkTheme, period, selectedCompanies, setSelectedCompanies, limit }) {
    ...
    if (loadingCompanies) {
        return (
            <div style={darkTheme ? styles.comparisonContainerDark : styles.comparisonContainer}>
                <p style={{ color: darkTheme ? '#94a3b8' : '#6b7280' }}>Loading companies...</p>
            </div>
        );
    }
