...
'''
Data module is using for write all the HTML page content IR (Inverstor Relastionship) 
and return all the links of the pdf files. 
then fillter manualy and download the desire files.
Its have to following method : 
    # write_html_content -> for read all the IR website content
    # read_html -> for preducing all the link for the file, can add filter
    # manipulate_links -> for changing the filtered_link to suit download.
    # download_pdf -> for downloading all the filltered link
...
def read_html(website_url, html_file_path, filter_keywords):
    ...
    return filtered_link_file.name


def manipulate_links(file_path, old_phrase, new_phrase):
    '''
    This function addresses cases where websites change the filenames on their servers, impacting content access. 
    By understanding the correct URL structure, this function modifies the links to make them suitable for downloading. 
    It takes a text file containing URLs, replaces a specific phrase in these URLs with a new one, and saves the modified URLs back to the file.
    '''
    ...
    # Save the modified links back to the file or to a new file
